(ns legalcompliance.actor
  "LegalComplianceActor — the ISCO-08 2619 community legalcompliance actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one legalcompliance operation request
  ...
  The unconditional invariant: the LegalComplianceAdvisor can never
  directly commit a record the LegalComplianceGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [legalcompliance.advisor :as advisor]
            [legalcompliance.governor :as governor]
            [legalcompliance.store :as store]))

(defn build-graph
  "Build a compiled LegalComplianceActor graph. `store` implements
  `legalcompliance.store/Store`. `advisor` implements
  `legalcompliance.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :document-id (:document-id proposal)
                                    :payload proposal}]
